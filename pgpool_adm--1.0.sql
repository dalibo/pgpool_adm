 ...
 */
CREATE FUNCTION pcp_proc_info(text, integer, integer, text, text, OUT database text, OUT username text,
    OUT start_time timestamptz, OUT creation_time timestamptz, OUT proto_major int2, OUT proto_minor int2,
    OUT counter integer, OUT backend_pid integer, OUT connected boolean, OUT PID integer, OUT backend_id integer)
RETURNS record
 ...
 */
CREATE FUNCTION pcp_proc_info(text, OUT database text, OUT username text, OUT start_time timestamptz,
    OUT creation_time timestamptz, OUT proto_major int2, OUT proto_minor int2, OUT counter integer,
    OUT backend_pid integer, OUT connected boolean, OUT PID integer, OUT backend_id integer)
RETURNS record
AS 'MODULE_PATHNAME', '_pcp_proc_info'
LANGUAGE C VOLATILE STRICT;
